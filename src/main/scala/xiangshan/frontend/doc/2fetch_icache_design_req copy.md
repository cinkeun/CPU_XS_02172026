...
`acquireArb` is a single `Arbiter(8+1)`, so only one acquire fires per cycle regardless of how many MSHRs are simultaneously ready. This serialization is the mechanism that naturally prevents victim way collision: `replacer.victim.req` fires at most once per cycle, and the replacer's next-cycle touch completes before the next acquire can fire. No additional victim arbitration logic is needed.
